        ...
        while (!window.shouldClose()) {
            ...
            if (ImGui::CollapsingHeader("Extra Features")) {
                ...
                if (config.features.extra.enableDepthOfField) {
                    ...
                    float minSize = 0.0f, maxSize = 1.0f;
                    float minLength = 0.0f, maxLength = 2.0f;
                    uint32_t minSamples = 1u, maxSamples = 64u;
                    ImGui::SliderScalar("Lens size", ImGuiDataType_Float, &config.features.extra.lensSize, &minSize, &maxSize);
                    ImGui::SliderScalar("Focal length", ImGuiDataType_Float, &config.features.extra.focalLength, &minLength, &maxLength);
                    ImGui::SliderScalar("Number of samples", ImGuiDataType_U32, &config.features.extra.numSamplesDOF, &minSamples, &maxSamples);
                    ImGui::Unindent();
                }
            ...
            case ViewMode::Rasterization: {
                ...
                if (debugBVHLevel || debugBVHLeaf) {
                    ...
                    enableDebugDraw = true;
                    if (debugBVHLevel)
                        bvh.debugDrawLevel(bvhDebugLevel, 0 , 0, config.features);
                    if (debugBVHLeaf) {
                        int currentLeaf = 0;
                        bvh.debugDrawLeaf(bvhDebugLeaf, currentLeaf, 0, config.features);
                    }
                    enableDebugDraw = false;
                    glPopAttrib();
                }
