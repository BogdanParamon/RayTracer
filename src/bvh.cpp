...
BVH::BVH(const Scene& scene, const Features& features)
{
    ...
    // Initialize the number of leaves and levels
    m_numLeaves = 0;
    m_numLevels = 1;

    // Build BVH
    buildRecursive(scene, features, primitives, RootIndex, m_numLevels);
...
{
    glm::vec3 minCoordinates = glm::min(glm::min(primitive.v0.position, primitive.v1.position), primitive.v2.position);
    glm::vec3 maxCoordinates = glm::max(glm::max(primitive.v0.position, primitive.v1.position), primitive.v2.position);
...
{
    glm::vec3 minCoordinates = primitives[0].v0.position;
    glm::vec3 maxCoordinates = primitives[0].v0.position;

    for (const BVHInterface::Primitive& p : primitives) {
        minCoordinates = glm::min(minCoordinates, glm::min(glm::min(p.v0.position, p.v1.position), p.v2.position));
        maxCoordinates = glm::max(maxCoordinates, glm::max(glm::max(p.v0.position, p.v1.position), p.v2.position));
    }
...
AxisAlignedBox computeCentroidSpanAABB(std::span<const BVHInterface::Primitive> primitives) {

    glm::vec3 minCoordinates = computePrimitiveCentroid(primitives[0]);
    glm::vec3 maxCoordinates = computePrimitiveCentroid(primitives[0]);

    for (const BVHInterface::Primitive& p : primitives) {
        minCoordinates = glm::min(minCoordinates, computePrimitiveCentroid(p));
        maxCoordinates = glm::max(maxCoordinates, computePrimitiveCentroid(p));
    }
...
uint32_t computeAABBLongestAxis(const AxisAlignedBox& aabb)
{
    ...
    glm::vec3 diagonal = aabb.upper - aabb.lower;
...
bool intersectRayWithBVH(RenderState& state, const BVHInterface& bvh, Ray& ray, HitInfo& hitInfo)
{
    ...
    if (state.features.enableAccelStructure) {
        ...
        stack.push(BVH::RootIndex);

        while (!stack.empty()) {
            uint32_t index = stack.top();
            stack.pop();
            const BVHInterface::Node& node = nodes[index];

            if (node.isLeaf()) {
                for (uint32_t i = node.primitiveOffset(); i < node.primitiveOffset() + node.primitiveCount(); i++) {
                    const BVHInterface::Primitive& p = primitives[i];
...
BVH::Node BVH::buildLeafData(const Scene& scene, const Features& features, const AxisAlignedBox& aabb, std::span<Primitive> primitives)
{
    ...
    node.data[0] = (Node::LeafBit | (uint32_t)m_primitives.size());
...
//
// You are free to modify this function's signature, as long as the constructor builds a BVH
void BVH::buildRecursive(const Scene& scene, const Features& features, std::span<Primitive> primitives, uint32_t nodeIndex, uint32_t currentLvl)
{
    ...
    AxisAlignedBox centroidAABB = computeCentroidSpanAABB(primitives);
    ...
    if (primitives.size() <= LeafSize) {
        m_nodes[nodeIndex] = buildLeafData(scene, features, aabb, primitives);
        m_numLevels = std::max(m_numLevels, currentLvl);
        m_numLeaves ++;
    }
    else {
        if (features.extra.enableBvhSahBinning == true) {
            int axis = computeAABBLongestAxis(centroidAABB);
            size_t median = splitPrimitivesBySAHBin(centroidAABB, axis, primitives);

            std::span<Primitive> left = primitives.subspan(0, median);
            std::span<Primitive> right = primitives.subspan(median);
            ...
            buildRecursive(scene, features, left, leftIndex, currentLvl + 1);
            buildRecursive(scene, features, right, rightIndex, currentLvl + 1);


        } else {
            int axis = computeAABBLongestAxis(aabb);
            size_t median = splitPrimitivesByMedian(aabb, axis, primitives);
...
void BVH::debugDrawLevel(int level, int currentLevel, int nodeIndex, const Features& features)
{
    ...
    if (level == currentLevel) {
        if (features.extra.enableBvhSahBinning)
            drawAABB(m_nodes[nodeIndex].aabb, DrawMode::Filled, glm::vec3(0.65f, 0.25f, 0.55f), 0.3f);
        drawAABB(m_nodes[nodeIndex].aabb, DrawMode::Wireframe, glm::vec3(0.05f, 1.0f, 0.05f), 1.0f);
    } else {
        if (!m_nodes[nodeIndex].isLeaf()) {
            debugDrawLevel(level, currentLevel + 1, m_nodes[nodeIndex].leftChild(), features);
            debugDrawLevel(level, currentLevel + 1, m_nodes[nodeIndex].rightChild(), features);
        }
    }
}
...
// You are free to modify this function's signature.
void BVH::debugDrawLeaf(int leafIndex, int& currentLeaf, int node, const Features& features)
{
    // Example showing how to draw an AABB as a (white) wireframe box.
    ...
    if (m_nodes[node].isLeaf() && leafIndex == currentLeaf) {
        if (features.extra.enableBvhSahBinning)
            drawAABB(m_nodes[node].aabb, DrawMode::Filled, glm::vec3(0.65f, 0.25f, 0.55f), 0.3f);
        drawAABB(m_nodes[node].aabb, DrawMode::Wireframe, glm::vec3(0.05f, 1.0f, 0.05f), 1.0f);
    } else {
        if (!m_nodes[node].isLeaf()) {
            debugDrawLeaf(leafIndex, currentLeaf, m_nodes[node].leftChild(), features);
            debugDrawLeaf(leafIndex, currentLeaf, m_nodes[node].rightChild(), features);
        }
    }
}