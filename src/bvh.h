...
private: // Private methods
    ...
    void buildRecursive(const Scene& scene, const Features& features, std::span<Primitive> primitives, uint32_t nodeIndex, uint32_t currentLvl);
...
public: // Visual debug
    ...
    void debugDrawLevel(int level, int currentLevel, int nodeIndex, const Features& features);

    // Draw data of the leaf at the selected index.
    // For a description of the method's arguments, refer to 'bounding_volume_hierarchy.cpp'
    // You are free to modify this function's signature.
    void debugDrawLeaf(int leafIndex, int& currentLeaf, int node, const Features& features);
